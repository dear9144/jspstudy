...
public class CookieServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		  Cookie
		  1. 서버가 만들어서 클라이언트가 저장 
		  2. 보안에 취약 
		 */
		
		// 쿠키 만들기 
		Cookie cookie1 = new Cookie("name", "신의정");
		...
		// 쿠키가 저장될 경로 설정 (생략하면 컨텍스트 패스에 저장됨 
		cookie1.setPath("/01_Servlet"); //컨텍스트 패스: request.getContextPath
		cookie2.setPath("/01_Servlet/CookieServlet1");  //경로(서블릿 경로) (/01_Servlet/CookieServlet1
														//cookie3의 경로는 생략됐다 
		
		// 쿠키 유효시간 '설정하기(생략하면 세셩쿠키가 된다. : 브라우저를 닫을 때까지 보관된다.)
		cookie1.setMaxAge(60 * 60 * 24 * 7); //7일간 보관되는 쿠키 
		cookie2.setMaxAge(60 * 60); //1시간동안 보관되는 쿠키 , 쿠키 시간은 숫자 1이 1초의미
